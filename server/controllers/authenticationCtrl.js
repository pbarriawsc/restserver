...
exports.postToken = (req, res) => {
    ...
    client.query('SELECT u.*,r.nombre as fk_rol_nombre,r.fk_bodega FROM public.usuario u inner join public.roles r on r.id=u.fk_rol where u.usuario = $1', [req.body.usuario], function (err, result) {
        if (err) {
            console.log(err);
        ...
        if(result.rows && result.rows.length>0){
            if(bcrypt.compareSync(req.body.password,result.rows[0].password)){//generando token y resguardando los campos que se envian para el encriptado
               const usuario={id:result.rows[0].id,
                ...
                fk_bodega:result.rows[0].fk_bodega
                };
                let token=jwt.sign({
                    usuario
