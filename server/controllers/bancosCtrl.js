...
exports.Post = async (req,res) =>{
    ...
    try {
        ...
        if ( !req.body.codigo || req.body.codigo.trim()==0 ) {
            ...
            var ExisteNombre = await client.query(`SELECT * from public.bancos where nombre='`+nombre+`' `);
            var ExisteCodigo = await client.query(`SELECT * from public.bancos where codigo='`+codigo+`' `);
            ...
            if(ExisteCodigo.rows.length>0) {
                ...
                await client.query(` INSERT INTO public.bancos (`+columna+`) VALUES (`+valor+`) `);
...
exports.Delete = async (req,res) =>{
    ...
    try {
        if ( !req.params.id || req.params.id==0 ) {
            ...
            await client.query(` DELETE FROM public.bancos WHERE id=`+parseInt(req.params.id)+` `);
            res.status(200).send([]); res.end(); res.connection.destroy();
        }
    } catch (error) {
        console.log("ERROR "+error);
...
exports.Get = async (req,res) =>{
    let token= req.get('Authorization'); jwt.verify(token, process.env.SECRET, (err,decoded)=>{ if(err){ return res.status(401).json({ success:false, err }) } req.usuario = decoded.usuario; });
    try {
          var Lista = await client.query(` SELECT * FROM public.bancos WHERE id=`+parseInt(req.params.id)+` `);
          res.status(200).send(Lista.rows); res.end(); res.connection.destroy();
    } catch (error) {
        console.log("ERROR "+error);
        res.status(400).send({
...
exports.Put = async (req,res) =>{
    ...
    try {
        if ( !req.body.id || req.body.id==0 ) {
            ...
            var ExisteNombre = await client.query(`SELECT * from public.bancos where id!=`+id+` and nombre='`+nombre+`' `);
            var ExisteCodigo = await client.query(`SELECT * from public.bancos where id!=`+id+` and codigo='`+codigo+`' `);
            ...
            if(ExisteNombre.rows.length>0) {
                ...
                await client.query(` UPDATE public.bancos SET `+valores+` where id=`+id);
                res.status(200).send([]); res.end(); res.connection.destroy();
            }
        }
    } catch (error) {
