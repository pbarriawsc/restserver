...
exports.Post = async (req,res) =>{
    ...
    try {
        if ( !req.body.nombre || req.body.nombre.trim()==0 ) {
            ...
            var Existe = await client.query(`SELECT * from public.cuentas_bancarias where nombre='`+nombre+`' `);
            if(Existe.rows.length>0) {
                ...
                await client.query(` INSERT INTO public.cuentas_bancarias (`+columna+`) VALUES (`+valor+`) `);
...
exports.Delete = async (req,res) =>{
    ...
    try {
        if ( !req.params.id || req.params.id==0 ) {
            ...
            await client.query(` DELETE FROM public.cuentas_bancarias WHERE id=`+parseInt(req.params.id)+` `);
            res.status(200).send([]); res.end(); res.connection.destroy();
        }
    } catch (error) {
        console.log("ERROR "+error);
...
exports.Get = async (req,res) =>{
    let token= req.get('Authorization'); jwt.verify(token, process.env.SECRET, (err,decoded)=>{ if(err){ return res.status(401).json({ success:false, err }) } req.usuario = decoded.usuario; });
    try {
          var Lista = await client.query(` SELECT * FROM public.cuentas_bancarias WHERE id=`+parseInt(req.params.id)+` `);
          res.status(200).send(Lista.rows); res.end(); res.connection.destroy();
    } catch (error) {
        console.log("ERROR "+error);
        res.status(400).send({
...
exports.Put = async (req,res) =>{
    ...
    try {
        if ( !req.body.nombre || req.body.nombre.trim()==0 ) {
            ...
            var id = req.body.id;
            var nombre = req.body.nombre.trim();
            var Existe = await client.query(`SELECT * from public.cuentas_bancarias where nombre='`+nombre+`' and id!=`+id+` `);
            if(Existe.rows.length>0) {
                res.status(400).send({
                message: "EL NOMBRE YA ESTA INGRESADO",
                success:false }); res.end(); res.connection.destroy();
                ...
                await client.query(` UPDATE public.cuentas_bancarias SET `+valores+` where id=`+id);
                res.status(200).send([]); res.end(); res.connection.destroy();
            }
        }
    } catch (error) {
