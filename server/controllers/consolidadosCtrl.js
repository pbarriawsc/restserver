...
exports.listTrackingConsolidadoByClient = (req, res) => {
  ...
  exports.listByClient = (req, res) => {
    ...
    client.query('SELECT c.*,u.nombre as fk_usuario_nombre, u.apellidos as fk_usuario_apellidos,cl."razonSocial" as fk_cliente_nombre, pc."cantProveedores",(SELECT count(id) FROM public.consolidado_tracking_detalle WHERE fk_consolidado=c.id)::integer AS bultos,(SELECT SUM (t.cantidad_bultos ) as total from  public.tracking t inner join public.consolidado_tracking ct on ct.fk_tracking=t.id where ct.fk_consolidado=c.id)::integer as bultos_programados,(SELECT count(td.id) FROM public.tracking_detalle td inner join public.tracking t on t.id=td.tracking_id inner join public.consolidado_tracking ct on ct.fk_tracking=t.id WHERE ct.fk_consolidado=c.id and td.estado=1)::integer AS bultos_completos,(SELECT count(t.id) FROM public.tracking t inner join public.consolidado_tracking ct on t.id=ct.fk_tracking WHERE ct.fk_consolidado=c.id and t.estado>0)::integer AS trackings_completos,(SELECT count(t.id) FROM public.tracking t inner join public.consolidado_tracking ct on t.id=ct.fk_tracking WHERE ct.fk_consolidado=c.id)::integer AS trackings_totales FROM public.consolidado c inner join usuario u on u.id=c.fk_usuario inner join public.clientes cl on cl.id=c.fk_cliente left join gc_propuestas_cabeceras pc on pc.id=c.fk_propuesta where c.fk_cliente=$1 ORDER BY c.id DESC', [parseInt(req.params.id)], function (err, result) {
        if (err) {
            console.log(err);
            res.status(400).send(err);
        }
...
//INSERCIÓN DE NUEVOS DATOS TIPO TRACKING EN LA TABLA DE CONSOLIDADOS
  ...
  exports.listGcConsolidadoByClient = (req, res) => {
    if (!req.params.id) {
     ...
     client.query('SELECT pc.* from public.gc_propuestas_cabeceras pc WHERE pc.fk_cliente=$1 AND pc.id>1 AND PC.estado!=999 AND pc."fechaValidez">=now()::date order by pc.id desc', [req.params.id], function (err, result) {
            if (err) {
                console.log(err);
                res.status(400).send(err);
            }   
  ...
  exports.updateStateToCharge = (req, res) => {
     if (!req.params.id) {
      res.status(400).send({
        message: "El id del consolidado es obligatorio",
        success:false
