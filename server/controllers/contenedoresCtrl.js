...
exports.list = (req, res) => {
    client.query('SELECT c.*,(SELECT count(id) FROM public.contenedor_historial WHERE fk_contenedor=c.id)::integer AS historial,n.nave_nombre as fk_nave_nombre,ne.eta_fecha,ne.eta_hora,ne.etd_fecha,ne.etd_hora,ln.nombre as fk_linea_naviera_nombre,e.nombre as fk_embarcadora_nombre,(SELECT ne.etd_Fecha FROM naves_eta ne LEFT JOIN contenedor_viajes_detalle cvd on cvd.fk_nave_eta=ne.id LEFT JOIN contenedor_tracking ct on ct.id=cvd.fk_contenedor_tracking WHERE ct.fk_contenedor=c.id order by ne.id asc limit 1) as fecha_salida,(SELECT ne.eta_Fecha FROM naves_eta ne LEFT JOIN contenedor_viajes_detalle cvd on cvd.fk_nave_eta=ne.id LEFT JOIN contenedor_tracking ct on ct.id=cvd.fk_contenedor_tracking WHERE ct.fk_contenedor=c.id order by ne.id desc limit 1) as fecha_llegada FROM public.contenedor c LEFT JOIN public.naves2 n on n.nave_id=c.fk_nave LEFT JOIN public.naves_eta ne on ne.id=c.fk_nave_eta LEFT JOIN public.lineas_navieras ln on ln.id=c.fk_linea_naviera LEFT JOIN public.embarcadoras e on e.id=c.fk_embarcadora ORDER BY id DESC', "", function (err, result) {
        if (err) {
            console.log(err);
